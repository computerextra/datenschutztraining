...
export default function UploadImage() {
  ...
  const saveFile = api.image.create.useMutation();
  const saveFileFromUrl = api.image.createFromUrl.useMutation();

  const fileInputRef = useRef<HTMLInputElement>(null);

  const handleSubmit = async () => {
    console.log("Start");
    if (selectedUrl != null && filename != null) {
      console.log("Upload Files from URL");
      const res = await saveFileFromUrl.mutateAsync({
        url: selectedUrl,
        fileName: filename,
      });
      if (res) router.push("/Admin/Medien/Bilder");
      else setError("Fehler beim speichern der Datei");
    }
    ...
    <div className="flex items-center justify-center">
      <div className="w-full max-w-5xl rounded-lg p-8 shadow-lg">
        <div className="mb-4 grid grid-cols-2 gap-4">
          ...
          <Button onClick={handleSubmit}>URL Upload</Button>
        </div>
